...
SkillType NaoBehavior::selectSkill() {
    ...
    worldModel->getRVSender()->clear(); // erases drawings from previous cycle   //画点
...
SkillType NaoBehavior::demoKickingCircle() {
    ...
    for(int i = WO_TEAMMATE1; i < WO_TEAMMATE1+NUM_AGENTS; ++i) {
        ...
        if (worldModel->getUNum() == playerNum) {
            ...
            if (teammate->validPosition) //判断看到信息是否正确
             {
                temp = teammate->pos;
            } else {
                continue;
        ...
        temp.setZ(0);  //将临时坐标的z坐标设置为0

        double distanceToBall = temp.getDistanceTo(ball); //判断球和人的距离
        if (distanceToBall < closestDistanceToBall) {//如果距离小于之前记录的最小距离(closestDistanceToBall)，更新最近球员的编号(playerClosestToBall)和最小距离(closestDistanceToBall)。
    ...
    if (playerClosestToBall == worldModel->getUNum()) {
        // Have closest player kick the ball toward the cente    r
        ...
        target = collisionAvoidance(true /*teammate*/, false/*opponent*/, true/*ball*/, 1/*proximity thresh*/, .5/*collision thresh*/, target, true/*keepDistance*/); //避免碰撞

        worldModel->getRVSender()->clear(); // erases drawings from previous cycle   //画点
        worldModel->getRVSender()->drawPoint("ball",target.getX(), target.getY(), 10.0f, RVSender::MAGENTA);
